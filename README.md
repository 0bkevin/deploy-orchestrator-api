...
## Production on a self-hosted Ubuntu VPS

For this service, run Bun under `systemd` on one Ubuntu VPS and place Caddy in front for TLS, request limits, and reverse proxying. Store the SQLite file under `/var/lib/deploy-orchestrator`, owned by the service user and separate from versioned application releases. Before deployment, CI should run a frozen installation, typecheck, lint, tests, and migration checks; the validated release can then be deployed using the environment's chosen method. For an application-only release, start the new version on a second port, wait for `/health`, switch Caddy to it, and keep the previous release ready for rollback. A backward-incompatible schema change should use a short maintenance window and a tested backup instead of a blue-green switch. Litestream can continuously copy SQLite changes to off-host object storage; regular restore tests should verify those backups. Use structured logs through `journald`, an external `/health` check, and alerts for disk space, failed backups, and rising error rates. Replace SQLite with PostgreSQL before adding multiple application hosts or sustained concurrent writes.
